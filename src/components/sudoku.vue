<template>
  <div> 
    ...
    <div class = "wrapper"> 
   <div class = "gameBoard">   <div class = "list" v-for="(number,index) in gameNums" :key="index">
        <div class = "cell=empty" v-if="number == 0"> 
         <!--<input  type="text" v-on:change="changeVal(index)" v-model="tempval" v-bind:id="'empty'+ index""> -->
         <select v-on:change="changeVal(index)" v-model="tempval" v-bind:id="'empty' + index"> 
        ...
        <div class = "cell" v-else b> {{number}} </div>
      </div> </div>
    <br>
    <br> 
